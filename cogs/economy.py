...
from economy_utils import dboperations
...
class Economy(commands.Cog, description="Economy commands."):
    ...
    def __init__(self, client: commands.Bot):
        ...
        self.connection = dboperations.connection()
    ...
    async def on_ready(self):
        ...
        dboperations.create_tables(self.connection)

    @commands.command(aliases=["bal", "money", "networth"])
    async def balance(self, ctx: commands.Context, member: discord.Member = None):
        embed = self.economy_embed(ctx)
        if member is None:
            if dboperations.check_member_exists(self.connection, ctx.author):
                money = dboperations.get_member_money(self.connection, ctx.author)
            ...
            if dboperations.check_member_exists(self.connection, member):
                money = dboperations.get_member_money(self.connection, member)
    ...
    async def addmoney(self, ctx: commands.Context, amount: int, member: discord.Member = None):
        ...
        if member is None:
            if dboperations.check_member_exists(self.connection, ctx.author):
                money = dboperations.add_member_money(self.connection, ctx.author, amount)
            ...
            if dboperations.check_member_exists(self.connection, member):
                money = dboperations.add_member_money(self.connection, member, amount)
    ...
    async def removemoney(self, ctx: commands.Context, amount: int, member: discord.Member = None):
        ...
        if member is None:
            if dboperations.check_member_exists(self.connection, ctx.author):
                networth = dboperations.get_member_money(self.connection, ctx.author)
                if amount > networth:
                    amount = networth
                money = dboperations.add_member_money(self.connection, ctx.author, -amount)
    ...
    async def work(self, ctx: commands.Context):
        embed = self.economy_embed(ctx)
        if dboperations.check_member_exists(self.connection, ctx.author):
            rank = dboperations.get_member_rank(self.connection, ctx.author)
            minmax = dboperations.get_rank_minmax_salary(self.connection, rank)
            salary = random.randint(minmax[0], minmax[1])
            operation = dboperations.add_member_money(self.connection, ctx.author, salary)
    ...
    async def createaccount(self, ctx: commands.Context):
        embed = self.economy_embed(ctx)
        rank = dboperations.get_rank_with_position(self.connection, 1)
        if not rank:
            embed.description = "A staff member needs to set up the default rank before members can create an account."
            return await ctx.send(embed=embed)
        operation = dboperations.create_member(self.connection, ctx.author.id, ctx.author.name, self.starting_money, rank[0][1])
    ...
    async def deleteaccount(self, ctx: commands.Context):
        embed = self.economy_embed(ctx)
        operation = dboperations.delete_member(self.connection, ctx.author.id)
    ...
    async def createrank(self, ctx: commands.Context, rank: str, minsalary: int, maxsalary: int, required: int = 0, position: int = 1):
        embed = self.economy_embed(ctx)
        operation = dboperations.create_rank(self.connection, rank.lower(), minsalary, maxsalary, required, position)
    ...
    async def deleterank(self, ctx: commands.Context, rank):
        embed = self.economy_embed(ctx)
        default_rank = dboperations.get_default_rank(self.connection)[0][1]
        if rank == default_rank:
            ...
            return await ctx.send(embed=embed)
        operation = dboperations.delete_rank(self.connection, rank.lower())
    ...
    async def profile(self, ctx: commands.Context, member: discord.Member = None):
        ...
        if member is None:
            if dboperations.check_member_exists(self.connection, ctx.author):
                member_data = dboperations.get_member(self.connection, ctx.author)[0]
    ...
    async def leaderboard(self, ctx: commands.Context):
        embed = self.economy_embed(ctx)
        embed.description = ""
        members_data = dboperations.get_leaderboard(self.connection)
        print(members_data)
